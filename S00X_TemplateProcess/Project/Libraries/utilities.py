...
def print_json_pretty(json_txt: str) -> None:
    """
    Print JSON text in console in readable format
    """
    try:
        # Format JSON
        json_formatted_str = json.dumps(json_txt, indent=5)
        # Print JSON
        pprint.pprint(json_formatted_str)
    except Exception as error_message:
        raise Exception(f"Method 'print_json_pretty' fails - {error_message}")
